...
const BedInfoAdmin = () => {
    ...
    const fetchWardHistory = async (patientId) => {
        try {
            const response = await fetch(`http://localhost:5000/patientsWardHistory/${patientId}`);
            if (!response.ok) {
    ...
    const fetchPatientInfo = async (patientId) => {
        try {
            const response = await fetch(`http://localhost:5000/patientInfoforWard/${patientId}`);
            if (!response.ok) {
