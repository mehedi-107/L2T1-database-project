...
const LeaveRequestList = () => {
  ...
  useEffect(() => {
    const fetchLeaveRequests = async () => {
      setLoading(true);
      try {
        const response = await fetch('http://localhost:5000/displayLeaveApplications');
        if (!response.ok) {
  ...
  const confirmApproveAction = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/manage${position}Leave/${id}`, {
        method: 'POST',
        headers: {
  ...
  const fetchAdditionalInfo = async (applicantId) => {
    const position = getPosition(applicantId);
    const url = `http://localhost:5000/applicantInfo/${applicantId}`;
    try {
