...
const Notification = ({ onClose }) => {
  ...
  const userId = 0;
  const { recentNotifications, fetchRecentNotifications } = useRecentNotifications(userId, currentPage);
  ...
  const handleSend = async () => {
    try {
      const response = await fetch('/sendMessage', {
        ...
        body: JSON.stringify({
          from: userId, // Replace 'User' with the actual sender ID or name
          to: to,
          message: message,
        }),
      });
