...
const Notification = ({ patientId, onClose }) => {
  const [message, setMessage] = useState('');
  const [to, setTo] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const [notificationsPerPage] = useState(4); // Display 4 notifications per page
  const { recentNotifications, fetchRecentNotifications } = useRecentNotifications(patientId, currentPage);
  ...
  const handleSend = async () => {
    try {
      const response = await fetch('/sendMessage', {
        ...
        body: JSON.stringify({
          from: patientId, // Replace 'User' with the actual sender ID or name
          to: to,
          message: message,
        }),
      });
